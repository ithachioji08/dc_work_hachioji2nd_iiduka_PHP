...
function updateQty($pdo,$id,$stock){
	...
	$updateSql = "UPDATE ec_cart SET product_qty=".$stock.",update_date=CURRENT_TIMESTAMP WHERE product_id = ".$id;
...
function oneDelete($pdo,$id){
	$pdo->beginTransaction();
	$sql = "DELETE from ec_cart where product_id =".$id;
